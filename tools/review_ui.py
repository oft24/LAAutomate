...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="laautomate-ui-") as temporal, ExitStack() as pila:
        ...
        from app.windows.assistant_view import _CrearAutomatizacionDialog
        dialogo = _CrearAutomatizacionDialog("reporte_semanal", ventana)
        dialogo.show()
        QTest.qWait(180)
        app.processEvents()
        dialogo.grab().save(str(salida / "crear-automatizacion.png"))
        dialogo.close()
        print("RENDERED", 19, "synthetic views", flush=True)
